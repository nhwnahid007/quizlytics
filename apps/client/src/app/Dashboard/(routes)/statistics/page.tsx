...
const Page = () => {
  ...
  const pieChartData = [
    { name: 'Low (0-40)', value: marks.filter((item) => (item.marks ?? 0) <= 40).length },
    { name: 'Medium (41-70)', value: marks.filter((item) => (item.marks ?? 0) > 40 && (item.marks ?? 0) <= 70).length },
    { name: 'High (71-100)', value: marks.filter((item) => (item.marks ?? 0) > 70).length },
  ];
