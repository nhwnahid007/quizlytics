...
const SubmitCard = ({
  ...
  return (
    <div className="relative group">
      <Card className="overflow-hidden border-border bg-white shadow-sm hover:shadow-md transition-all duration-300 rounded-3xl">
        ...
        <CardContent className="px-8 pb-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {item.options.map((option: string, index: number) => {
              ...
              let variantClasses = "bg-muted/30 border-transparent text-muted-foreground";
              if (isSelected) {
                variantClasses = isCorrect
                  ? "bg-green-50 border-green-200 text-green-700 font-semibold ring-1 ring-green-200"
                  : "bg-red-50 border-red-200 text-red-700 font-semibold ring-1 ring-red-200";
              } else if (isCorrectOption && !isCorrect) {
                // Highlight the correct answer if the user got it wrong
                variantClasses = "bg-green-50/50 border-green-100 text-green-600 font-medium";
              }
              ...
              return (
                ...
                >
                  ...
                  {isCorrectOption && !isCorrect && (
                    <AiOutlineCheckCircle className="text-green-500/50 text-lg shrink-0" title="Correct Answer" />
                  )}
                </div>
              );
            })}
        ...
        <CardFooter className="px-8 py-6 bg-muted/20 border-t border-border/50">
          <div className="w-full">
            <div className="flex items-center gap-2 mb-2">
              <div className={`h-2 w-2 rounded-full ${isCorrect ? 'bg-green-500' : 'bg-red-500'}`} />
            ...
            {item.explain && (
              <div className="mt-3 p-4 rounded-2xl bg-white border border-border/50">
                <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-1">Explanation</p>
