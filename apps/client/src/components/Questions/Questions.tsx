...
const Questions = ({ currentMCQ, setCurrentMCQ, userExamData, setUserExamData, examId, setExamId, allMCQ, setShowResult }: QuestionsProps) => {
    const [exactMCQ, setExactMCQ] = useState<QuizQuestion | null>(null);

    useEffect(() => {
        const getOneExpected = () => {
            const expectedOne = allMCQ.find((question) => Number(question.id) === currentMCQ);
